...
url_guardant_ccf <- str_c(
	"../",
	"data/",
	"razavi_all_reported_variants_adjusted.csv"
	)

url_gmaf_ctdna_frac_v1 <- str_c(
		"../",
		"data/",
		"gmaf_ctdna_frac.csv"
	)
	
url_gmaf_ctdna_frac <- str_c(
		"../",
		"data/",
		"gmaf_ctdna_frac_v2_05062019.csv"
	)

url_facets_key <- str_c(
		"../",
		"data/",
		"summary_pedram.csv"
	)

url_gmaf_to_annotate <- str_c(
		"../",
		"data/",
		"byl_guardant_maf.Rda"
	)
...
url_byl_clinical <- str_c(
		"../",
		"data/",
		"byl_clinical.Rda"
	)
	
url_gmaf_oncokb_annotated <- str_c(
		"../",
		"data/",
		"gmaf_oncokb.annotated.maf"
	)
	
duplicate_id <- list(
	sample_id = c('008_B','037_B','042 C27', '044 C1D8','047_B'))

nomut_nopik_x <- list(
	sample_id = c("044 C28", "044_C32", "049 B2",
				  "052_C18", "061_C2D15", "013 C39",
				  "013_C1", "014_B", "018_B",
				  "018_C1D15", "028_C1D1", "029_C1D15",
				  "036 C1D1", "051 C13", "051 C1D1", "051 end",
				  "052 B", "061 B2"))
		
nomut_nopik_y <- list(
	sample_id = c("044_C28", "044_C32", "049_B2",
				  "052_C18", "061_C2D15", "013_C39",
				  "013_C1",  "014_B", "018_B",
				  "018_C1D15", "028_C1D1", "029_C1D15",
				  "036_C1D1", "051_C13", "051_C1D1",
				  "051_end", "052_B", "061_B2"))
					  
max_vaf <- data_frame(
	sample_id =   c("002", "004", "006", "007", "008",
					"009", "016", "021", "022", "023",
					"025", "026", "029", "031", "032",
					"033", "034", "035", "038", "039",
					"040", "042", "043", "045", "046",
					"047", "048", "050", "053", "054",
					"057", "059", "060", "030", "037"),
	hgvsp_short = c("PIK3CA_E545K", "PIK3CA_E542K", "ESR1_Y537N", "PIK3CA_H1047R", "PIK3CA_H1047R",
					"PIK3CA_E542K", "PTEN_R130*", "PIK3CA_E545K", "PIK3CA_E970K", "PIK3CA_H1047R",
					"PIK3CA_H1047R", "PIK3CA_E545K", "PIK3CA_E542K", "PIK3CA_Q546P", "PIK3CA_H1047L",
					"PIK3CA_H1047R", "PIK3CA_E542K", "BRCA2_E215del", "PIK3CA_H1047R", "PIK3CA_H1047R",
					"PIK3CA_H1047R", "FGFR1_R91L", "PIK3CA_H1047R", "PIK3CA_H1047R", "PIK3CA_E545K",
					"PIK3CA_H1047Y", "PIK3CA_E542K", "PIK3CA_E542K", "PIK3CA_E545K", "PIK3CA_E545K",
					"PIK3CA_Q546R", "PIK3CA_E545A", "PIK3CA_E545K", "PIK3CA_N345K", "ESR1_D538G"))
...
'col_distinct' <- function(n=60)
{
	qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
	col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
	return(invisible(col_vector))
}

'fun_lm_ccf' <- function(data)
{
	index = data[,"ccf_pre"] > .99 & data[,"ccf_post"] > .99
	if (sum(index)==0) {
		index = data[,"ccf_pre"] > .98 & data[,"ccf_post"] > .98
	}
	if (sum(index)==0) {
		index = data[,"ccf_pre"] > .98 & data[,"ccf_post"] > .98
	}
	if (sum(index)==0) {
		index = data[,"ccf_pre"] > .97 & data[,"ccf_post"] > .97
	}
	if (sum(index)==0) {
		index = data[,"ccf_pre"] > .90 & data[,"ccf_post"] > .90
	}
	data = data[index,,drop=FALSE]
	fit = lm(ccf_post ~ ccf_pre +0, data=data)
	return(invisible(fit))
}


'fun_lm_vaf' <- function(data)
{
	index = data[,"ccf_pre"] > .99 & data[,"ccf_post"] > .99
	if (sum(index)==0) {
		index = data[,"ccf_pre"] > .98 & data[,"ccf_post"] > .98
	...
	data = data[index,,drop=FALSE]
	fit = lm(vaf_post ~ vaf_pre +0, data=data)
	return(invisible(fit))
}

'fun_lm_vaf_2' <- function(max_vaf = NULL, data, sample_id)
{
	if (is.null(max_vaf)) {
		max_vaf = base::get("max_vaf", envir = globalenv())
	}
	index = max_vaf$hgvsp_short[max_vaf$sample_id == sample_id]
	index = data[,"uuid"] == index
	if (sum(index)==0 && sample_id=="035") {
			index = "BRCA2_p.Glu215del"
			index = data[,"uuid"] == index
	}
	data = data[index,,drop=FALSE]
	fit = lm(vaf_post ~ vaf_pre +0, data=data)
						 ...
						 ...) 
{
    ...
    if (method == "circle" && plotCI == "n") {
    	...
    	if (!is.null(border.col)) {
    		symbols(Pos, add = TRUE, inches = FALSE, circles = asp_rescale_factor * 0.9 * abs(DAT)^0.5/2, fg = col.border, bg = col.fill)
    	} else {
    		symbols(Pos, add = TRUE, inches = FALSE, circles = asp_rescale_factor * 0.9 * abs(DAT)^0.5/2, fg = border.col, bg = col.fill)
    	}
    }
    ...
    if (method == "color" && plotCI == "n") {
        corrplot:::draw_method_color(Pos, "white", col.fill)
    }
    ...
    if (cl.pos != "n") {
        ...
        colorlegend(colbar = colbar, labels = round(labels, 2), offset = cl.offset, ratio.colbar = 0.3, cex = cl.cex, xlim = xlim, ylim = ylim, vertical = vertical, align = cl.align.text)
    }
    if (tl.pos != "n") {
        ...
        if (tl.pos == "d") {
            ...
            text(pos.ylabel[, 1] + 0.5, pos.ylabel[, 2], newcolnames[1:min(n, 
                m)], col = tl.col, cex = tl.cex, font=3, ...)
        }
        else {
            text(pos.xlabel[, 1], pos.xlabel[, 2], newcolnames, 
                srt = tl.srt, adj = ifelse(tl.srt == 0, c(0.5, 
                  0), c(0, 0)), col = tl.col, cex = tl.cex, offset = tl.offset, font=1,
                ...)
            text(pos.ylabel[, 1], pos.ylabel[, 2], newrownames, 
                col = tl.col, cex = tl.cex, pos = 2, offset = tl.offset, font=3,
                ...)
        }
    }
    title(title, ...)
